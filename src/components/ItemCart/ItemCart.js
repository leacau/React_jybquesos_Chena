...
const ItemCart = ({id, tipo, marca, img, precio, cantidad }) => {
    ...
    <div className="detalleItemCart rounded">
        ...
        <div><span>Producto: </span>{ tipo } ({ marca })</div>
        <div><span>Precio xKg: </span>${ precio }</div>
        <div><span>Cantidad: </span>{ cantidad }</div>
        <div><span>Subtotal: </span>${ subtotal }</div>
        <button onClick={deleteItem} className='btn btn-danger p-1 m-3'><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" className="bi bi-trash" viewBox="0 0 16 16">
